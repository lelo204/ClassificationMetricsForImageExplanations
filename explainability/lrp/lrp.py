...
import sys
#sys.path.append(r'C:\Users\aditi\OneDrive\Desktop\Focus_Metric\explainability')
...
class LRP(Explainability):    
    def __init__(self, model):
        super().__init__('lrp')
        self.model = model.to(self.device)      # GPU edit
    ...
    def get_scores(_tensor):
        ...
        scores = np.array(x.data.view(-1).cpu())
        return scores

    def compute_heatmap(self, img, target_category):
        input = img.to(self.device)         # GPU edit
        input.requires_grad = True
        output = self.model(input)
        scores = self.get_scores(output)
        n_outputs = len(scores)
        _tensor = self.compute_tensor(output, n_outputs, target_category).to(self.device)
