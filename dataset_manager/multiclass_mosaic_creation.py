...
def mosaic_creation(dataset: DatasetArgs, mosaic: MosaicArgs, mosaics_per_class: int, seed: Union[int, None] = None):
    ...
    #val_df = dataset_df[dataset_df['subset'] == 'val']                                  # ignored because we compute focus for all mosaics and not only validation subset

    idx_mosaic = 0
    all_labels = list(dataset_df['label'].map(label2idx).unique())

    for target_class in all_labels:
        outer_classes = all_labels.copy()
        outer_classes.remove(target_class)

        #target_class_df = val_df[val_df['label_idx'] == target_class]                  # ignored because we compute focus for all mosaics and not only validation subset      
        target_class_df = dataset_df[dataset_df['label_idx'] == target_class]
        ...
        for outer_class in outer_classes:
            #outer_class_df = val_df[val_df['label_idx'] == outer_class]                # ignored because we compute focus for all mosaics and not only validation subset
            outer_class_df = dataset_df[dataset_df['label_idx'] == outer_class] 
